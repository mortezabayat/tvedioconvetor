...
import ir.bayat.morteza.tvideoconvertor.video.InputSurface;
import ir.bayat.morteza.tvideoconvertor.video.MP4Builder;
import ir.bayat.morteza.tvideoconvertor.video.Mp4Movie;
import ir.bayat.morteza.tvideoconvertor.video.OutputSurface;
...
public class MediaController {
    ...
    public void setBaseActivity(Activity activity, Handler applicationHandler) {
        baseActivity = activity;
        this.applicationHandler = applicationHandler;
    }

    public void scheduleVideoConvert(VideoObject videoObject) {
    ...
    private boolean scheduleVideoConvert(VideoObject videoObject, boolean isEmpty) {
        if (isEmpty && !videoConvertQueue.isEmpty()) {
            ...
            new File(videoObject.attachPath).delete();
        }
        videoConvertQueue.add(videoObject);
    ...
    public void cancelVideoConvert(VideoObject videoObject) {
        if (videoObject == null) {
            ...
            if (!videoConvertQueue.isEmpty()) {
                if (videoConvertQueue.get(0) == videoObject) {
                    synchronized (videoConvertSync) {
                        cancelCurrentVideoConversion = true;
                    }
                } else {
    ...
    private boolean startVideoConvertFromQueue() {
        if (!videoConvertQueue.isEmpty()) {
            ...
            VideoObject videoObject = videoConvertQueue.get(0);
//            Intent intent = new Intent(baseActivity, VideoEncodingService.class);
//            intent.putExtra("path", videoObject.attachPath);
...
//                if (error) {
//                    NotificationCenter.getInstance().postNotificationName(NotificationCenter.FilePreparingFailed, videoObject, file.toString());
//                } else {
//                    if (firstWrite) {
//                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.FilePreparingStarted, videoObject, file.toString());
//                    }
...
//                }
    ...
    private static class VideoConvertRunnable implements Runnable {
        ...
        @Override
        public void run() {
            MediaController.getInstance().convertVideo(videoObject);
        }

        static void runConversion(final VideoObject obj) {
    ...
    /**
     * @param videoObject
     * @return false if encounter error and true is convection is success
     */
    private boolean convertVideo(final VideoObject videoObject) {
        String videoPath = videoObject.videoEditedInfo.originalPath;
        long startTime = videoObject.videoEditedInfo.startTime;
        long endTime = videoObject.videoEditedInfo.endTime;
        int resultWidth = videoObject.videoEditedInfo.resultWidth;
        int resultHeight = videoObject.videoEditedInfo.resultHeight;
        int rotationValue = videoObject.videoEditedInfo.rotationValue;
        ...
        SharedPreferences preferences = baseActivity.getSharedPreferences("videoconvert", Activity.MODE_PRIVATE);
        File inputFile = new File(videoPath);
        if (videoObject.getId() != 0) {
        ...
        if (resultWidth != 0 && resultHeight != 0) {
            ...
            try {
                ...
                if (resultWidth != originalWidth || resultHeight != originalHeight || rotateRender != 0 || videoObject.videoEditedInfo.roundVideo) {
                    int videoIndex;
                    ...
                    if (videoIndex >= 0) {
                        ...
                        try {
                            ...
                            if (Build.VERSION.SDK_INT < 18) {
                                ...
                                FileLog.e("codec = " + codecInfo.getName() + " manufacturer = "
                                        + manufacturer + "device = " + Build.MODEL);
                            } else {
